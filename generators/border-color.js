...
export default {
    handle: (className, prefix, suffix) => {
        if (!className.startsWith("border-color-")) return null;

        try {
            let parts = className.split("-");
    
            let colorValue = colors[parts[2]];
            if (colorValue == null) return null;

            let lightness = (parts.length > 3) ? parts[3] : colorValue.l;
    
            return (`.${prefix}${className}${suffix} { border: 1px solid hsl(${colorValue.h}, ${colorValue.s}%, ${lightness}%); }`);
        } catch (ex) { }
        return null;
    }
}